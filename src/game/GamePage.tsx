...
import {
  ChevronLeft, BookOpen, X, Star, TrendingUp,
  Wrench, Clock, AlertCircle, User, ExternalLink,
} from 'lucide-react';
import {
  type Game, type GameWinner, type GameVolatility,
  fetchGameBySlug, fetchGamesByCategory, fetchWinnersByGame,
} from './data';

const volatilityLabels: Record<GameVolatility, string> = {
  low: 'Низкая',
  medium: 'Средняя',
  high: 'Высокая',
...
function WinnerAvatar({ winner, size }: { winner: GameWinner; size: 'sm' | 'md' }) {
  const dim = size === 'sm' ? 'w-6 h-6' : 'w-7 h-7';
  const iconSize = size === 'sm' ? 11 : 13;
  if (winner.player?.avatar) {
    return (
      <img
        src={winner.player.avatar}
        alt=""
        className={`${dim} rounded-full object-cover shrink-0 border border-white/15`}
      />
    );
  }
  return (
    <div className={`${dim} rounded-full bg-white/10 border border-white/15 flex items-center justify-center shrink-0`}>
      <User size={iconSize} className="text-white/40" />
    </div>
  );
}

function WinnerName({ winner }: { winner: GameWinner }) {
  return winner.player?.nickname || winner.name || 'Аноним';
}

function WinnerTxLink({ winner }: { winner: GameWinner }) {
  if (!winner.tx_signature) return null;
  const href = `https://explorer.solana.com/tx/${winner.tx_signature}?cluster=devnet`;
  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex items-center gap-1 text-white/30 hover:text-white/60 transition-colors shrink-0"
...
function WinnersList({ winners, variant }: { winners: GameWinner[]; variant: 'sidebar' | 'grid' | 'mobile' }) {
  if (winners.length === 0) {
    return <p className="text-white/30 text-[12px] text-center py-6">Нет данных о выигрышах</p>;
  }
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-2.5 max-h-[200px] overflow-y-auto">
        {winners.map((w) => (
          <div key={w.id} className="flex items-center gap-2.5">
            <WinnerAvatar winner={w} size="sm" />
            <div className="min-w-0 flex-1">
              <span className="text-white text-[11px] font-bold truncate block"><WinnerName winner={w} /></span>
  ...
  return (
    <div className="flex flex-col gap-2">
      {winners.map((w) => (
        <div key={w.id} className="flex items-center gap-2.5">
          <WinnerAvatar winner={w} size="md" />
          <div className="min-w-0 flex-1">
            <span className="text-white text-[12px] font-bold truncate block"><WinnerName winner={w} /></span>
            <span className="text-white/30 text-[10px] truncate block">{w.game}</span>
          </div>
          <span className="text-white font-black text-[11px] tabular-nums shrink-0">{w.amount}</span>
          <WinnerTxLink winner={w} />
...
function LoadingState({ onHome }: { onHome: () => void }) {
  return (
    <div className="flex justify-center w-full flex-1 px-1 pt-3 pb-6">
      <div className="w-full max-w-[1408px] flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)_300px] gap-3 items-start">
          <Skeleton className="hidden lg:block h-[280px] rounded-xl" />
          <div className="flex flex-col gap-3">
            <Skeleton className="h-[440px] rounded-xl" />
            <Skeleton className="h-16 rounded-xl" />
          </div>
          <Skeleton className="hidden lg:block h-[200px] rounded-xl" />
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex justify-center w-full flex-1 px-1 pt-3 pb-6">
      <div className="w-full max-w-[1408px]">
        ...
        <div className="bg-black border border-white/15 rounded-xl p-12 text-center">
          <AlertCircle size={40} className="text-white/30 mx-auto mb-4" strokeWidth={1.5} />
          <p className="text-white/40 text-[15px] font-semibold">Игра не найдена</p>
          <button onClick={onHome} className="mt-5 bg-white text-black font-black text-[12px] tracking-[0.2em] rounded-md px-6 py-3 hover:bg-white/90 transition-colors">
            НА ГЛАВНУЮ
          </button>
        </div>
      </div>
...
function MaintenanceState({ game, onHome }: { game: Game; onHome: () => void }) {
  return (
    <div className="flex justify-center w-full flex-1 px-1 pt-3 pb-6">
      <div className="w-full max-w-[1408px] flex flex-col gap-3">
        <div className="flex items-center gap-3">
          ...
          <nav className="flex items-center gap-1.5 text-[11px] font-bold tracking-wide">
            ...
            <span className="text-white/40">{game.provider}</span>
            <span className="text-white/20">/</span>
...
export default function GamePage({
    ...
    fetchGameBySlug(gameId)
      .then(async (g) => {
        ...
        const [wins, recs] = await Promise.all([
          fetchWinnersByGame(g.id, 10).catch(() => [] as GameWinner[]),
          fetchGamesByCategory(g.category, g.id, 7).catch(() => [] as Game[]),
        ]);
        if (cancelled) return;
        setWinners(wins);
        setRecommended(recs);
  ...
  if (loading) return <LoadingState onHome={onHome} />;
  if (notFound || !game) return <NotFoundState onHome={onHome} />;
  if (game.status === 'maintenance') return <MaintenanceState game={game} onHome={onHome} />;
  if (game.status === 'disabled') return <NotFoundState onHome={onHome} />;

  const rtpStr = `${game.rtp.toFixed(2)}%`;
  const maxWinStr = `x${game.max_win}`;
  const minBetStr = `${game.min_bet.toLocaleString('ru-RU')} ₽`;
  const maxBetStr = `${game.max_bet.toLocaleString('ru-RU')} ₽`;
  const volStr = volatilityLabels[game.volatility] ?? game.volatility;
  ...
  return (
    <div className="flex justify-center w-full flex-1 px-1 pt-3 pb-6">
      <div className="w-full max-w-[1408px] flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)_300px] gap-3 items-start">
          ...
          <aside className="hidden lg:flex flex-col gap-3">
            {winners.length > 0 && (
              <div className="bg-black border border-white/20 rounded-xl p-4">
                ...
                <WinnersList winners={winners.slice(0, 5)} variant="sidebar" />
              </div>
            )}
          ...
          <div className="flex flex-col gap-3 min-w-0">
            ...
            <div className="relative bg-[#0a0a0a] border border-white/20 rounded-xl overflow-hidden h-[280px] sm:h-[360px] lg:h-[440px]">
              ...
              {/* Status badge */}
              <div className="absolute top-3 left-3 flex items-center gap-2 z-10">
                <span className="flex items-center gap-1.5 bg-green-500/20 border border-green-500/40 rounded-full px-2.5 py-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-green-500" />
                  <span className="text-green-400 text-[10px] font-black tracking-widest">АКТИВНА</span>
                </span>
              </div>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-5 px-6 text-center">
                <div
                  ...
                  style={{ background: `${game.accent}15`, border: `1px solid ${game.accent}33` }}
                >
                  <span className="text-2xl font-black" style={{ color: game.accent }}>{game.name.charAt(0)}</span>
                </div>
                <div>
                  <p className="text-white font-black text-[22px] tracking-wide">{game.name}</p>
                  <p className="text-white/40 text-[13px] font-semibold mt-1">{game.provider}</p>
                ...
                <button
                  ...
                  className="bg-white text-black font-black text-[12px] tracking-[0.2em] rounded-md px-8 py-3.5 hover:bg-white/90 transition-colors"
                >
                  ИГРАТЬ
                </button>
              </div>
            ...
            <div className="lg:hidden">
              ...
              {mobileTab === 'info' ? (
                <div className="bg-black border border-white/15 rounded-xl p-4">
                  <div className="flex flex-col gap-2 text-[11px]">
                    <div className="flex justify-between">
                      ...
                      <span className="text-white font-bold">{rtpStr}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/40">Макс. выигрыш</span>
                      <span className="text-white font-bold">{maxWinStr}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/40">Мин. ставка</span>
                      <span className="text-white font-bold">{minBetStr}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/40">Макс. ставка</span>
                      <span className="text-white font-bold">{maxBetStr}</span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className="bg-black border border-white/15 rounded-xl p-3 max-h-[240px] overflow-y-auto">
                  <WinnersList winners={winners} variant="mobile" />
                </div>
              )}
            </div>
            ...
            {winners.length > 0 && (
              <div className="hidden lg:block bg-black border border-white/15 rounded-xl p-4">
                ...
                <WinnersList winners={winners} variant="grid" />
              </div>
            )}
          </div>
        ...
        {/* Description */}
        {(game.full_description || game.description || (game.features && game.features.length > 0)) && (
          <div className="bg-black border border-white/15 rounded-xl p-5 md:p-6">
            ...
            <p className="text-white/60 text-[13px] leading-relaxed">{game.full_description || game.description}</p>
        ...
        {recommended.length > 0 && (
          <div>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-thin">
              {recommended.map((g) => (
                <GameCard key={g.id} game={g} onPlay={(gg) => onOpenGame(gg.slug)} />
              ))}
            </div>
          </div>
        )}
        ...
        <div className="sm:hidden sticky bottom-0 z-30 bg-black/95 backdrop-blur-md border border-white/15 rounded-xl p-3 -mx-1">
          <div className="flex items-center gap-2 mb-2">
            <div
              ...
              style={{ background: `${game.accent}22`, border: `1px solid ${game.accent}44` }}
            >
              <span className="text-sm font-black" style={{ color: game.accent }}>{game.name.charAt(0)}</span>
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-white text-[12px] font-bold truncate">{game.name}</p>
              <p className="text-green-400 text-[10px] font-bold tracking-wide">Активна</p>
            </div>
            <button
              onClick={() => setShowRules(true)}
          ...
          <div className="bg-black border border-white/20 rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-4">
              {game.rules.length > 0 ? (
                ...
                <p className="text-white/30 text-[13px]">Правила для этой игры пока не добавлены.</p>
              )}
              {game.paytable && game.paytable.length > 0 && (
                <div>
