...
import { fetchPopularGames, type Game } from '@/game/data';
    ...
    <div className="flex-1 min-w-0 flex flex-col gap-4 md:gap-6 pb-4 lg:overflow-y-auto">
          ...
          {winners.map((w) => {
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-white font-bold text-[13px] truncate">{nickname}</p>
      ...
      {gamesLoading ? (
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-5 gap-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="animate-pulse bg-white/5 border border-white/10 rounded-xl aspect-[3/4]" />
            ))}
          </div>
        </div>
      ) : games.length > 0 ? (
        ...
        <div>
          ...
          <div className="bg-black border border-white/15 rounded-xl p-10 text-center">
            <p className="text-white/30 text-[13px] font-semibold">Скоро здесь появятся игры</p>
          </div>
        </div>
      )}
    </div>
